...
contract SalaryCalculator {
    ...
    modifier validRate(uint256 _rate) {
        require(_rate > 0, "Hourly rate must be larger than zero");
        _;
    }

    modifier validHoursWorked(uint256 _hours) {
        require(_hours > 0, "Hours worked must be larger than zero");
        _;
    }
    ...
    function calculateTotalSalary() public onlyOwner {
        require(hourlyRate > 0 && hoursWorked > 0, "Hourly Rate is Zero");
